...
add_max_snail_count <- all_inverts_mdr %>%
  ...
  summarize(common_species = str_c(snail_species_max, collapse = ", ")) %>%

#error: invert_species doesn't exist
...
animate(max_species_per_quadrat, duration = 10, renderer = gifski_renderer("max_species_per_quadrat.gif"))

# laurie ? -> (1) how to squash y-axis to make quadrats 'square' (scale in theme?); (2) when multiple species are the most abundant within a quadrat (common_species), make a new color of that combination

#add in 2017
#fix lag -> change duration within animation
#condense code into one pipeline and one ggplot
