...
```{r abundant-snails-animation, eval = FALSE}

add_max_snail_count <- all_inverts_mdr %>%
  filter(year != 2017) %>%
  filter(invert_species %in% c("Lacu_v", "Litt_l", "Litt_o", "Litt_s", "Nuce_l")) %>%
  ...
  rename(snail_species = invert_species)

add_max_snail_species_1 <- add_max_snail_count %>%
  filter(max_count == 0) %>%
  mutate(snail_species_max = snail_species == NA)

add_max_snail_species_2 <- add_max_snail_count %>%
  filter(max_count != 0) %>%
  mutate(snail_species_max = snail_species)


mostabundant_snail_species_per_quadrat <- rbind(add_max_snail_species_1, add_max_snail_species_2)%>%
  select(-snail_species) %>%
  ...
  theme(axis.title.y = element_blank(),
    ...
    labs(title = "Year {closest_state}",
         fill = "Snail Species") +
  scale_fill_viridis_d(
    labels = c(expression(italic("Lacu_v" = "Lacuna vincta")),
               expression(italic("Litt_l" = "Littorina littorea")),
               expression(italic("Litt_o" = "Littorina obtusata")),
               expression(italic("Litt_s" = "Littorina saxatilis")),
               expression(italic("Nuce_l" = "Nucella lapillus")),
               "NA"),
    na.value = "grey70") +
...
# laurie ? -> (1) how to fix lag in animation between title & graph change; (2) transition_length & state_ don't seem to be working...too long; (3) how to squash y-axis to make quadrats 'square' (scale in theme?); how to make set up to animation a single pipeline/how to condense code (combine everything above ggplot)
