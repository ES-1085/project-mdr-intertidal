...
```{r L-mean-seaweed-cover-graph-faceted, fig.height = 4, fig.width = 7}

# how to make spp. un-italicized?
# remove U. lactuca in high quadrats
# use this one for presentation
...
seaweed_labs <- c("Ascophyllum nodosum",
                  "Ceramium rubrum",
                  "Chondrus crispus",
...
expanded_seaweeds_mdr %>%
  filter(seaweed_simple %in% c("Asco_n",
                               "Cera_r",
                               "Chon_c",
                               "Cora_o",
                               "Fucu_spp",
  ...
  summarise(sum_proportion = sum(proportion), sd = sd(proportion), n = n()) %>%
  ...
  ggplot(mapping = aes(year, mean_proportion, fill = seaweed_simple)) +
    ...
    scale_fill_viridis_d(labels = c("Asco_n" = "Ascophyllum nodosum",
