...
seaweeds_mdr_2022 <- horizontal_MDR_NeCSA_2022 %>%
  ...
  pivot_longer(
    ...
    values_to = "squares out of 25") %>%
  mutate(proportion = `squares out of 25`/25) %>%
  mutate_at(c("proportion"), ~na_if(., 0)) %>%
  mutate("Quadrat (m)" = case_match(Quadrat, 1 ~ 0, 2 ~ 3, 3 ~ 6, 4 ~ 9, 5 ~ 12, 6 ~ 15, 7 ~ 18, 8 ~ 21, 9 ~  24, 10 ~ 27)) %>%
  relocate("Quadrat (m)", .after = TideHt) %>%
  select(-c("Quadrat", "squares out of 25")) %>%
  rename(Tide = TideHt)
